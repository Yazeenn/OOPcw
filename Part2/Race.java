...
public class Race
{
    ...
    public String getWinningHorseName() {
        return winningHorseName;
    }

    public String getAverageSpeed() {
        // Check if there's a winner and if time was recorded
        if (winningHorseName != null && winTime > 0) {
            // Convert raceLength to meters and winningTime to seconds

            double averageSpeed = raceLength / winTime;
            String averageSpeedFormatted = String.format("%.2f", averageSpeed);

            // Calculate average speed in meters per second
            return( "Average speed of winning horse: " + averageSpeedFormatted + " m/s");
        } else {
            // If there's no winner or no recorded time, return -1 to indicate no average speed
            return "No winner, no average speed";
        }
    }
    ...
    public void startRace() {
        ...
        winningHorseName = null; // Reset winning horse's name
        ...
        while (!finished) {
            ...
            try {
                ...
                winTime += 0.1; // for calculating average speed, tracking the time
            } catch (Exception e) {
            }
        }
    }
    ...
    private boolean raceWon()
    {
        for (Horse horse : lanes) {
            if (horse != null && horse.getDistanceTravelled() == raceLength) {
                winningHorseName = horse.getName(); // Set the winning horse's name
